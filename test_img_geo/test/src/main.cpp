...
int main(int argc, char *argv[])
{
  ...
  case 'j':
    if (!igl::readOBJ(std::string(DATA_SHARED_PATH "/") + std::string(argv[1]), V, F)) {
      std::cerr << "Couldn't load object" << std::endl;
      exit(1);
    }
    break;
  case 'f':
    if (!igl::readOFF(std::string(DATA_SHARED_PATH "/") + std::string(argv[1]), V, F)) {
      std::cerr << "Couldn't load object" << std::endl;
      exit(1);
    }
    break;
  ...
  {
    if(!igl::heat_geodesics_precompute(V_img,F_img,t,data))
    {
