...
int main(int argc, char *argv[])
{
  ...
  // Measure throughput of copying the distance map to and from the device
  {
     std::stringstream ss;
     ss << "Measure copying " << cols * rows * sizeof(Scalar) << " bytes (pitched: " << d_D_pitch_bytes[0] * rows << " bytes) to and from the device";
     std::string msg = ss.str();
     perfCuda.perfCopy2DTimes(d_D[0], d_D_pitch_bytes[0], p_D, cols * sizeof(Scalar), cols, rows,
       msg);
  }
