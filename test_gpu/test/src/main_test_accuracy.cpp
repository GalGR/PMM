...
// Heat geodesics
igl::HeatGeodesicsData<Scalar> heat_data;
Eigen::Matrix<Scalar, Eigen::Dynamic, 1> heat_D;

// Geodesics difference
Eigen::Matrix<Scalar, Eigen::Dynamic, 1> diff_D;
Scalar diff_min = {};
Scalar diff_max = {};
Scalar diff_min_scaled = {};
Scalar diff_max_scaled = {};
Scalar diff_min_heat_scaled = {};
Scalar diff_max_heat_scaled = {};
...
// Program parameters
std::string model_name = "test_disk.obj";
size_t rows;
size_t cols;
size_t img_len;
int harmonic_const;
...
bool update(update_method method = UPDATE_CLEAR) {
    ...
    viewer.core().proj, viewer.core().viewport, V_img, F_img, fid, bc))
  {
    ...
    // if(F_img.rows()>100000)
    {
      ...
      // Calculating Heat Geodesics
      TIMER_START("Running Heat Geodesics");
      igl::heat_geodesics_solve(heat_data, (Eigen::VectorXi(1,1)<<vid).finished(),heat_D);
      TIMER_END();
      TIMER_START("Calculating the squared difference between the two distances");
      assert(D.size() == heat_D.size());
      diff_D = Eigen::Matrix<Scalar, Eigen::Dynamic, 1>(D.size());
      for (unsigned i = 0; i < D.size(); ++i) {
        diff_D(i) = std::pow(D(i) - heat_D(i), 2);
      }
      TIMER_END();
      TIMER_START("Calculating minimums and maximums");
      diff_min = diff_D.minCoeff();
      diff_max = diff_D.maxCoeff();
      D_max = D.maxCoeff();
      heat_D_max = heat_D.maxCoeff();
      diff_min_scaled = std::sqrt(diff_min) / D_max;
      diff_max_scaled = std::sqrt(diff_max) / D_max;
      diff_min_heat_scaled = std::sqrt(diff_min) / heat_D_max;
      diff_max_heat_scaled = std::sqrt(diff_max) / heat_D_max;
      TIMER_END();
      TIMER_START("Updating distances");
      viewer.data().set_data(diff_D.cast<double>());
      TIMER_END();
    }
    // else
    // {
...
void set_colormap(igl::opengl::glfw::Viewer & viewer)
{
  ...
  const int num_intervals = 30;
  Eigen::MatrixXd CM(num_intervals,3);
  igl::colormap(igl::COLOR_MAP_TYPE_JET,Eigen::VectorXd::LinSpaced(num_intervals,0,1).eval(),0,1,CM);
...
void init_imgui()
{
  ...
  menu.callback_draw_viewer_menu = [](){
    ...
    // Add new group
    if (ImGui::CollapsingHeader("Algorithm Comparison", ImGuiTreeNodeFlags_DefaultOpen))
    {
      ImGui::Text("Max Error²: %.4f", diff_max);
      ImGui::Text("Max Dist PMM: %.4f", D_max);
      ImGui::Text("Max Dist Heat: %.4f", heat_D_max);
      ImGui::Text("Error ÷ PMM: %.2f%%", diff_max_scaled * 100.0);
      ImGui::Text("Error ÷ Heat: %.2f%%", diff_max_heat_scaled * 100.0);
    }
  };
}
...
int main(int argc, char *argv[])
{
  ...
  if (!vm.count("height") && !vm.count("width")) {
    std::cerr << "'height' and 'width' are required arguments" << std::endl;
    std::cout << desc << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  const auto precompute_heat = [&]()
  {
    double t = std::pow(igl::avg_edge_length(V_img,F_img), 2);
    if (!igl::heat_geodesics_precompute(V_img, F_img, heat_data)) {
      TIMER_ERROR("heat_geodesics_precompute failed");
      exit(EXIT_FAILURE);
    }
  };
  TIMER_START("Precomputing heat geodesics data");
  precompute_heat();
  TIMER_END();
  ...
  // Allocate memory for heat_D
  TIMER_START("Allocating memory for heat_D");
  {
    heat_D = Eigen::Matrix<Scalar, Eigen::Dynamic, 1>::Zero(V_img.rows());
  }
  TIMER_END();

  // Allocate memory for diff_D
  TIMER_START("Allocating memory for diff_D");
  {
    diff_D = Eigen::Matrix<Scalar, Eigen::Dynamic, 1>::Zero(V_img.rows());
  }
  TIMER_END();
  ...
  if (start_with_source) {
    ...
    Eigen::Matrix<Scalar, Eigen::Dynamic, 1> &D = diff_D;
    S = start_source;
    ...
    TIMER_END();
    TIMER_START("Running initial Heat Geodesics with " + start_source_str);
    Eigen::Matrix<unsigned, Eigen::Dynamic, 1> vector_start_source(start_source.size());
    for (unsigned i = 0; i < vector_start_source.size(); ++i) {
      vector_start_source(i) = start_source[i];
    }
    igl::heat_geodesics_solve(heat_data,vector_start_source,heat_D);
    TIMER_END();
    TIMER_START("Calculating the squared difference between the two distances");
    assert(D.size() == heat_D.size());
    diff_D = Eigen::Matrix<Scalar, Eigen::Dynamic, 1>(D.size());
