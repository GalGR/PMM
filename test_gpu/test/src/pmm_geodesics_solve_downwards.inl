        ...
        for (unsigned i = 1; i < tile_height && x < width; ++i) {
            if (y + i < height) {
                ...
                if (1 <= x && x <= width - 1 && 1 <= threadIdx.x && threadIdx.x <= tile_width - 1) { // Left triangle -- even rows coefficients
                    Scalar4 ab = *reinterpret_cast<Scalar4*>(&C[(x - 1) * PMM_COEFF_PITCH + (4 * (height - 1 - (y + i)) + 0 + 0) * C_pitch]);
                    Scalar4 c  = *reinterpret_cast<Scalar4*>(&C[(x - 1) * PMM_COEFF_PITCH + (4 * (height - 1 - (y + i)) + 0 + 1) * C_pitch]);
                ...
                if (0 <= x && x <= width - 2 && 0 <= threadIdx.x && threadIdx.x <= tile_width - 2) { // Right triangle -- odd rows coefficients
                    Scalar4 ab = *reinterpret_cast<Scalar4*>(&C[x * PMM_COEFF_PITCH + (4 * (height - 1 - (y + i)) + 2 + 0) * C_pitch]);
                    Scalar4 c  = *reinterpret_cast<Scalar4*>(&C[x * PMM_COEFF_PITCH + (4 * (height - 1 - (y + i)) + 2 + 1) * C_pitch]);
