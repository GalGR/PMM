...
__global__ void solve_downwards(
    ...
    for (unsigned xoffset = 0; xoffset < width + tile_offset; xoffset += xstride) {
        ...
        for (unsigned i = 1; i < tile_height && x < width; ++i) {
            if (y + i < height) {
                ...
                if (1 <= x && x <= width - 1 && 1 <= threadIdx.x && threadIdx.x <= tile_width - 1) { // Left triangle -- even rows coefficients
                    Scalar4 ab = *reinterpret_cast<Scalar4*>(&C[(PMM_A_OFF + 0) + (x - 1) * PMM_COEFF_PITCH + (2 * (height - 1 - (y + i))) * C_pitch]);
                    Scalar4 c  = *reinterpret_cast<Scalar4*>(&C[(PMM_C_OFF + 0) + (x - 1) * PMM_COEFF_PITCH + (2 * (height - 1 - (y + i))) * C_pitch]);
                ...
                if (0 <= x && x <= width - 2 && 0 <= threadIdx.x && threadIdx.x <= tile_width - 2) { // Right triangle -- odd rows coefficients
                    Scalar4 ab = *reinterpret_cast<Scalar4*>(&C[(PMM_A_OFF + 0) + x * PMM_COEFF_PITCH + (2 * (height - 1 - (y + i)) + 1) * C_pitch]);
                    Scalar4 c  = *reinterpret_cast<Scalar4*>(&C[(PMM_C_OFF + 0) + x * PMM_COEFF_PITCH + (2 * (height - 1 - (y + i)) + 1) * C_pitch]);
        ...
        for (unsigned i = 1; i < tile_height && x < width; ++i) {
            if (y + i < height && threadIdx.x >= i && threadIdx.x < i + tile_eff_width) {
                D[x + (height - 1 - (y + i)) * D_pitch] = d_shared[threadIdx.x + i * tile_pitch];
            }
        }
    }
}