        ...
        for (unsigned xoffset = 0; xoffset < width; xoffset += xstride) {
            ...
            for (unsigned i = 1; i < tile_height && x < width; ++i) {
                if (y + i < height) {
                    ...
                    if (x == width - 1 || threadIdx.x > 0 && threadIdx.x < tile_width) { // Left triangle -- even rows coefficients
                    ...
                    if (x == 0 || threadIdx.x > 0 && threadIdx.x < tile_width) { // Right triangle -- odd rows coefficients
